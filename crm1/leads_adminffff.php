<?php
require_once __DIR__ . '/../gocarparts-main/includes/config.php';
session_start();
...
<html lang="en">
  ...
  <?php else: ?>
    <?php
      ...
      $employeeName = htmlspecialchars($row['assigned_employee']);

      switch ($status) {
        case 'completed':
          echo "<span class='badge bg-success'>Completed by $employeeName</span>";
          break;
        case 'reassigned':
          echo "<span class='badge bg-warning text-dark'>Reassigned to $employeeName</span>";
          break;
        case 'assigned':
        default:
          echo "<span class='badge bg-info'>Assigned to $employeeName</span>";
          break;
      }
    ?>
  <?php endif; ?>
  ...
  Assign
        ...
        data-bs-toggle="modal" 
        data-bs-target="#assignModal" 
        onclick="openAssignModal(<?= $row['id'] ?>, '<?= $row['lead_type'] ?>', 'reassign')">
  Reassign
</button>
                  <?php endif; ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" id="modal_lead_type">

        <div class="mb-3">
          <label for="modal_employee_id" class="form-label">Select Employee</label>
          <select class="form-select" id="modal_employee_id" required>
            <option value="">Select Employee</option>
            <?php
            $users = $conn->query("SELECT id, username FROM users WHERE role = 'employee'");
            while ($emp = $users->fetch_assoc()) {
              echo "<option value='{$emp['id']}'>{$emp['username']}</option>";
            }
            ?>
          </select>
        </div>
        <div id="assign_status_msg" class="text-success fw-bold mt-2"></div>
      </div>
...
<!--  -->
  ...
  <!--end wrapper-->
    ...
    <script src="assets/js/main.js"></script>
             ...
             <script>
function openAssignModal(id, type, mode) {
  document.getElementById('modal_lead_id').value = id;
  document.getElementById('modal_lead_type').value = type;
  document.getElementById('assignModalLabel').innerText = mode === 'reassign' ? 'Reassign Lead' : 'Assign Lead';
  document.getElementById('assign_status_msg').innerText = '';
  document.getElementById('modal_employee_id').value = '';
}
function submitAssign() {
  const lead_id = document.getElementById('modal_lead_id').value;
  const lead_type = document.getElementById('modal_lead_type').value;
  const employee_id = document.getElementById('modal_employee_id').value;
  ...
  fetch('assign_task_admin1.php', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    },
    body: `lead_id=${encodeURIComponent(lead_id)}&lead_type=${encodeURIComponent(lead_type)}&employee_id=${encodeURIComponent(employee_id)}`
  })
  .then(res => res.text())
  .then(response => {
    const statusDiv = document.getElementById('assign_status_msg');
    
    if (response.trim() === 'success') {
      statusDiv.innerText = 'Lead successfully assigned!';
      setTimeout(() => location.reload(), 1000);
    } else {
      statusDiv.innerText = 'Assignment failed: ' + response;
    }
  })
  .catch(err => {
    console.error(err);
...
//       'Content-Type': 'application/x-www-form-urlencoded'
//     },
//     body: `lead_id=${lead_id}&lead_type=${lead_type}&employee_id=${employee_id}`
//   })
//   .then(res => res.text())
//   .then(response => {
//     document.getElementById('assign_task_admin1.php').innerText = 'Lead successfully assigned!';
//     setTimeout(() => {
//       location.reload(); // Refresh to reflect updated assignment
//     }, 1000);
//   })
//   .catch(err => {
//     console.error(err);
