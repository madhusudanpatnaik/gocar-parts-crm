...
As Requests 2.0.0 is a major release, this version contains breaking changes. There is an [upgrade guide](https://requests.ryanmccue.info/docs/upgrading.php) available to guide you through making the necessary changes in your own code.
...
setting the value of this constant to `true`.
  ...
  A complete "translation table" between the Requests 1.x and 2.x class names is available in the [upgrade guide](https://requests.ryanmccue.info/docs/upgrading.php).
...
- **Update bundled certificates**
  ...
  The [documentation regarding Secure Requests with SSL](https://requests.ryanmccue.info/docs/usage-advanced.php#secure-requests-with-ssl) has also been updated to reflect this and it is recommended to have a read through.
...
- **Manage `"Expect"` header with `cURL` transport**
  ...
  By default, `cURL` adds a `Expect: 100-Continue` header to certain requests. This can add as much as a second delay to requests done using `cURL`. This is [discussed on the cURL mailing list](https://curl.se/mail/lib-2017-07/0013.php).
...
[docs/proxy]: https://requests.ryanmccue.info/docs/proxy.php
[docs/usage-advanced]: https://requests.ryanmccue.info/docs/usage-advanced.php
